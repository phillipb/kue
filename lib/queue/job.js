...
//    console.log( "removeState(%d) START ", this.id, state, this._state );
    ...
    multi.zrem('q:jobs', this.id);
    multi.zrem('q:jobs:' + state, this.id);
    multi.zrem('q:jobs:' + this.type + ':' + state, this.id);
    multi.exec(function (err, replies) {
...
//        console.log( "setState(%d) Start ", this.id, state, this._state );
        var multi = client.multi();
        multi.zadd('q:jobs', this._priority, this.id);
        multi.zadd('q:jobs:' + state, this._priority, this.id);
        multi.zadd('q:jobs:' + this.type + ':' + state, this._priority, this.id);
        ('inactive' == state) ? multi.lpush('q:' + this.type + ':jobs', 1) : '';
        multi.exec(function (err, replies) {
